      ...
      if (key === 'site') {
        if (checkEmptyFields(key)) {
          ...
          if (!regex.test(data[key])) {
            ...
            node.insertAdjacentHTML(
              'afterend',
              `<p class=${styles.error}>A link to the site should start with https://</p>`
            );
            node.focus();
            return false;
          }
      ...
      if (key === 'phone') {
        if (checkEmptyFields(key)) {
          ...
          if (!regex.test(data[key])) {
            ...
            node.insertAdjacentHTML(
              'afterend',
              `<p class=${styles.error}>Please, enter valid phone number</p>`
            );
            node.focus();
            return false;
          }
          if (data[key].length > 12) {
            ...
            node.insertAdjacentHTML(
              'afterend',
              `<p class=${styles.error}>Your phone number should be less then 12</p>`
            );
            node.focus();
            return false;
          }
  ...
  return (
    <form className={styles.form} onSubmit={handleSubmit} onReset={handleReset}>
      ...
      <Textarea
        ...
        className='about'
        rows='7'
        placeholder='leave a little bit of information about youreself'
        value={data.about}
        onChange={handleChange}
      />
      <Textarea
